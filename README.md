...
>
> **Claude:** "Goal complete. I opened PR #142 and am waiting for Claude Code Action review."
>
> *(Two minutes later)*
>
...
>
> *(Claude fixes it. Review re-runs. Passes.)*
>
...
The runtime follows a simple rule set inspired by skill-first agent workflows:

- Hooks only trigger or inject context.
- The `review-loop` skill owns the PR workflow.
- Scripts are reserved for version synchronization and deterministic Superset file installation.
- The same Claude Code session fixes review feedback in the same worktree.
- GitHub Claude Code Action reviews; it does not repair.
- Missing, stale, or ambiguous review output is never treated as pass.
...
- You're OK letting the review-loop skill commit, push, open PRs, and enable auto-merge after the Goal is ready

❌ **Not a good fit (for now):**
- You're doing exploratory work and don't want the review loop to start yet
...
Step 2 (once per repo)     → onboard           │  in your project's MAIN folder
                              ...
                              onboarding skill │  on the default branch
                                               │
...
| **Either an existing GitHub repo, OR ready to make a new one** | `gh repo view` works inside the folder | The onboarding skill will help you choose the repo path |
...
> ls .git                 # this should exist (or the onboarding skill will stop and ask before init)
> ```

> ⚠️ **Why "main folder" matters:** the setup commits `.github/workflows/claude*.yml` to your repo. GitHub Actions can only authorize them when they're on the **default branch**, so they must land there first. If you run setup from a worktree, the plugin will tell you to switch back to the main folder.

Make sure you're on your repo's default branch:
...
The `github-actions-onboarding` skill does the rest. It owns the workflow and runs direct `gh`/`git` commands; no onboarding helper script is used. The flow:
...
2. **Locate your repo** — three sub-cases:
   ...
   - ⚠️ Local git repo but **no GitHub remote** → the skill discusses the shortest safe path and can run `gh repo create` if you choose that route.
   - ⚠️ Folder isn't even a git repo yet → the skill stops and asks you to confirm the repo initialization path first.
3. **Verify the Claude review bot is installed** — detected via the `check_suites` side-channel (no clicks needed if the App is already installed account-wide, which is the common case). If not detected, the skill sends you to the install URL.
4. **Set the `CLAUDE_CODE_OAUTH_TOKEN` secret** — the skill *cannot* run this automatically (both `claude setup-token` and `gh secret set` are interactive — browser OAuth + paste). It detects either a repo secret or an org secret visible to this repo. If missing, it prints both supported setup paths:
   ```bash
   # in your terminal — DO NOT paste the token into chat
   claude setup-token

   # Recommended for organizations: one secret covers every repo
   gh secret set CLAUDE_CODE_OAUTH_TOKEN --org <org> --visibility all

   # Optional selected-repo org rollout
   gh secret set CLAUDE_CODE_OAUTH_TOKEN --org <org> --repos <repo>

   # Fallback when you do not use org secrets
   gh secret set CLAUDE_CODE_OAUTH_TOKEN -R <owner>/<repo>
   ```
   Then the skill confirms the secret directly with `gh api` / `gh secret list`.
5. **Install Claude review workflow YAMLs** from plugin templates. Codex review is added only when you explicitly ask for Codex or both.
6. **Commit and push onboarding files** to the repo's default branch.
7. **Seed** `.claude/24hour-ClaudeCode.config.json` (your local knobs).
8. **Health check** — confirms all wiring.

After onboarding finishes, **do one thing on the GitHub website**: open your repo → Settings → General → ☑️ **Allow auto-merge**. Without this, PRs won't merge automatically after review passes.
...
> **Why a worktree?** Each worktree is a separate folder for a separate branch. The plugin **only activates inside worktrees**, so your main folder stays untouched — you can keep using Claude Code normally there for manual edits, exploration, or read-only work, and the review loop will not start there.
...
```
After updating, **restart Claude Code** (`/exit` then `claude` again) so the new hooks load. Check the installed version any time with `claude plugin list`. The workflow YAMLs in your project's `.github/workflows/` are *not* touched by a plugin update — re-run `/24hour-ClaudeCode:setup` if you want to reinstall the current templates.
...
The plugin uses Claude Code's **hook** system. SessionStart is a command hook because Claude Code does not support prompt hooks there; Stop routing is a prompt hook, and the `review-loop` skill also has a skill-scoped Stop prompt hook.

If you want to understand the architecture under the hood — which hooks fire when, why Stop is prompt-based, and how the skill owns the loop — see **[FLOW.md](FLOW.md)** ([中文版](FLOW.zh-CN.md)).
